          ...
          {!chat.ready ? null : chat.messages.length === 0 ? (
            ...
            <div className="space-y-3">
              {chat.messages.map((message) => (
                <MessageRow key={message.id} message={message} copied={copiedId === message.id} onCopy={() => copyMessage(message)} onRetry={() => void retry(message)} busy={chat.busy} />
              ))}
            </div>
          )}
          </div>
...
function MessageRow({ message, copied, onCopy, onRetry, busy }: { message: ChatMessage; copied: boolean; onCopy: () => void; onRetry: () => void; busy: boolean }) {
