...
export function systemLayer(input: SystemLayerInput): string {
  ...
  parts.push(
    [
      ...
      "- Competitor research: the msgate-crm MCP tool search_competitor_creatives returns a competitor's static Meta ads from Brand Search with the only signals Brand Search provides (EU spend and reach estimates, reach rank, days active, duplicate variants) and image URLs to look at (they expire in 3 days). Rank them as « top creatives based on available Brand Search signals », never as proven winners; never invent ROAS, revenue or CTR. Look at the images, cluster the recurring creative patterns, then adapt the mechanisms to the CRM product in context: keep angle, hook mechanism, layout, hierarchy and proof structure, replace every product fact with the CRM product's own, drop what has no equivalent, and never transfer the competitor's brand, product, packaging, claims, guarantees, origin, materials, certifications, studies or statistics. The operator chooses which ads to use; recommend, do not impose.",
